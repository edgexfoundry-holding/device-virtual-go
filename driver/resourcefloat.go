...
func (rf *resourceFloat) value(db *db, ro *models.ResourceOperation, deviceName, deviceResourceName, minimum,
	...
	err = db.updateResourceValue(strconv.FormatFloat(newValueFloat, 'e', -1, bitSize), data.DeviceName, data.DeviceResourceName)
	return result, err
}
...
func (rf *resourceFloat) write(param *dsModels.CommandValue, deviceName string, db *db) error {
	...
	case deviceResourceFloat32:
		if v, err := param.Float32Value(); err == nil {
			return db.updateResourceValue(strconv.FormatFloat(float64(v), 'e', -1, 32), deviceName, param.RO.Resource)
		} else {
			return err
		}
	case deviceResourceFloat64:
		if v, err := param.Float64Value(); err == nil {
			return db.updateResourceValue(strconv.FormatFloat(float64(v), 'e', -1, 64), deviceName, param.RO.Resource)
		} else {
			return err
		}
	default:
